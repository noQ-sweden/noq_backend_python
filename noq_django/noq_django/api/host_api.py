...
@router.get("/pending_count", tags=["host-frontpage"]) #Visa mängden aktiva förfrågningar
...
@router.get("/arrivals_count", tags=["host-frontpage"]) #Visa dagens arrivals
...
@router.get("/departures_count", tags=["host-frontpage"]) #Visa dagens departures
...
@router.get("/current_guests_count", tags=["host-frontpage"]) #Visa incheckade gäster
...
@router.get("/available_products", response=Dict[str, int], tags=["host-frontpage"]) #Visa tillgängliga sovplatser uppdelat i typ 
...
def get_pending_bookings(request):
    bookings = Booking.objects.filter(product__host=Host.objects.get(user=request.user))
    
    return bookings 

@router.get("/pending/{request_id}", response=BookingSchema, tags=["host-manage-requests"])
def detailed_pending_booking(request, booking_id: int):
    booking = get_object_or_404(Booking, id=booking_id, product__host=Host.objects.get(user=request.user))
